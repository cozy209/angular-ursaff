...
export class AppState {
    ...
    @Action(UpdateUsers)
    updateDataOfState(ctx: StateContext<UserStateModel>, { payload, i }: UpdateUsers) {
        return this._du.updateUser(payload, i).pipe(tap(() => {
            const state=ctx.getState();
    ...
    deleteDataFromState(ctx: StateContext<UserStateModel>, { id }: DeleteUsers) {
        return this._du.deleteUser(id).pipe(tap(() => {
            const state=ctx.getState();
